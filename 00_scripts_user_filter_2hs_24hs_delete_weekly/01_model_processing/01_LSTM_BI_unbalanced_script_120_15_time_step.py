...
outputMetricFile = "result_trad_"+str(NN_type)+"_"+outputFileSufixTrain+"_batch_size_"+str(BATCH_SIZE)+"_window_"+str(TIME_SERIES_SIZE)+"-"+str(TIME_STEP_SHIFT)+".csv"
...
for TEST_EPOCHS in EPOCHS_ARRAY_TEST:
    ...
    model.add(Bidirectional(LSTM(128,return_sequences=True),input_shape=(120, 9,)))
    model.add((LSTM(128)))
    ...
    # fit network
    model.fit(train_dataset_series, epochs=epochs, verbose=verbose, batch_size=batch_size) #, batch_size=batch_size, validation_split=0.1
    # evaluate model
